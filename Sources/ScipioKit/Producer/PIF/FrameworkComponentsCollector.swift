...
struct FrameworkComponentsCollector {
    ...
    func collectComponents(sdk: SDK) throws -> FrameworkComponents {
        ...
        switch buildOptions.frameworkModuleMapGenerationPolicy {
        case .autoGenerated:
            frameworkModuleMapPath = try generateFrameworkModuleMap()
        case .custom(let url):
            logger.info("📝 Using custom modulemap \(url.path()) for \(buildProduct.target.name)(\(sdk.displayName))")
            frameworkModuleMapPath = url.absolutePath
        }
